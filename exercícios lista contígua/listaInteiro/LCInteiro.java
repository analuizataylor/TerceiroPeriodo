...
public class LCInteiro {
    ...
    public LCInteiro concatena(LCInteiro outraLista) {
        LCInteiro novaLista = new LCInteiro(this.quant + outraLista.quant);

        for (int i = 0; i < this.quant; i++) {
            novaLista.insereFinal(this.lista[i]);
        }
        for (int i = 0; i < outraLista.quant; i++) {
            novaLista.insereFinal(outraLista.lista[i]);
        }
        return novaLista;
    }

    public boolean igual (LCInteiro outraLista){
        if (this.quant != outraLista.quant){
            return false;
        }
        for (int i = 0; i < this.quant; i++){
            if (!this.get(i).equals(outraLista.get(i))){
                return false;
            }
        }
        return true;
...
public boolean estaContida(LCInteiro outraLista) {
    for (int i = 0; i < this.quant; i++) {
        Integer elemento = this.lista[i];
        if (outraLista.pesquisa(elemento) == -1) {
            return false; 
        }
    }
    return true;
